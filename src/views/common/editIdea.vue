        ...
        <el-row class="info-area">
            <el-form :model="addIdeaform" :rules="rules" ref="addIdeaform" label-width="100px" class="form">
                ...
                <el-form-item class="btn-box">
                    ...
                    <el-button type="primary" class="confirm-btn" @click="saveForm('addIdeaform')">保存</el-button>
                </el-form-item>
            </el-form>
        </el-row>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      init () {
        this.addIdeaform.ideaId = this.$route.params.ideaId;
        this.$axios({
          url: '/addIdea/querIdeaByIdeaID',
          method: 'get',
          params: {
            ideaID: this.addIdeaform.ideaId
          }
        }).then(({data}) => {
          let idea = data.data;
          this.addIdeaform.ideaName = idea.ideaName;
          this.addIdeaform.type = idea.type;
          this.handleTypeChange(this.addIdeaform.type);
          this.imageUrl = 'http://39.105.55.23:8089/addsp/image/'+idea.imgUrl;
          this.addIdeaform.imgUrl = idea.imgUrl;
          this.addIdeaform.price = idea.ideaCost;
          this.addIdeaform.appId = idea.appId;
          this.addIdeaform.appCode = idea.appCode;
          this.addIdeaform.status = idea.status;
          this.addIdeaform.tempMedia = idea.advertCode;
          // this.getAdverterCodes();
          this.HandleAppIdChange(idea.appId);
          this.addIdeaform.advMedia = idea.advertCode;
        })
      },
      beforeSubmit () {
        let _this = this
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(this.addIdeaform.price < this.lowestPrice){
              ...
              let status = this.addIdeaform.status;
              if(status != 2){
                this.beforeSubmit();
                if (this.balance < 0) {
                  status = 4
              ...
              this.$axios({
                ...
                params: {
                  ...
                  'advertCode': this.addIdeaform.advMedia,
                  'status':status,
                  'ideaID':this.addIdeaform.ideaId
                }
              }).then(({data}) => {
          ...
          if (data.data) {
            ...
            for (let i = 0; i < this.adverterCodes.length; i++) {
              ...
              for (let j = 0; j < result.length; j++) {
                if (tempCode.value == result[j] && tempCode.value!=this.addIdeaform.tempMedia) {
                  sameIndex.push(i)
                }
              }
            };
